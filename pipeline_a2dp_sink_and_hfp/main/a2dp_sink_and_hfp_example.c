...
void a2dp_sink_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param) {
    switch (event) {
        case ESP_A2D_CONNECTION_STATE_EVT:
            if (param->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
                ...
                // Bluetooth audio device is disconnected
        ...
        case ESP_A2D_AUDIO_STATE_EVT:
            if (param->audio_stat.state == ESP_A2D_AUDIO_STATE_STARTED) {
                // Audio is playing
                ESP_LOGI(TAG,"Now playing audio!");
            } else if (param->audio_stat.state == ESP_A2D_AUDIO_STATE_STOPPED) {
                // Audio has stopped
                ESP_LOGI(TAG,"Stopping audio!");
            } else if (param->audio_stat.state == ESP_A2D_AUDIO_STATE_REMOTE_SUSPEND) {
                // Audio has been paused by the Bluetooth audio device
                ESP_LOGI(TAG,"Pausing audio!");
            } 
            // else if (param->audio_stat.state == ESP_A2D_AUDIO_STATE_REMOTE_STOP) {
            //     // Audio has been stopped by the Bluetooth audio device
            // }
            break;
        default:
            break;
...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT &&
            ...
            1)
            // (int) msg.data == ESP_AUDIO_PLAYER_STOPPED)
        {
            // Audio playback has stopped
            ESP_LOGI(TAG,"Status changed!");
        }
